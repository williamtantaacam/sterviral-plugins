...
if (!empty($sync_by_collections)) {
   ...
   if (!$DB_Products->is_product_in_selected_collections($data->id, $collection_ids)) {
      error_log('WP Shopify Warning: You have sync by collections enabled but your recently created product, "' . $data->title .'", does not belong to any collections. Therefore, it was not created in WordPress.');
      exit;
   }
