...
$collects_result    = $DB_Collects->modify_from_shopify(
                        $DB_Collects->modify_options(
                           $API_Items_Collects->get_collects_from_collection($data),
                           WP_SHOPIFY_COLLECTIONS_LOOKUP_KEY
                        )
                     );

$collection_result  = $DB_Collections->update_items_of_type( $DB_Collections->mod_before_change($data, $post_id) );

WP_Shopify\Transients::delete_cached_single_collection_by_id($post_id);
