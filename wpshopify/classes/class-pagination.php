	...
	*/
    public function construct_pagination_limits($query)
    {
        ...
        if (empty($wps_related_products)) {
            ...
            $wps_related_products_count = $query->get(
                'wps_related_products_count'
            );
	...
	*/
    public function get_page_number_link($args, $page)
    {
		...
		*/
        ...
        } elseif ($post_type === WP_SHOPIFY_COLLECTIONS_POST_TYPE_SLUG) {
            $urlPrefix =
                $homeURL . '/' . $general_settings->url_collections . '/';
        }
    ...
    public function get_max_pages($args)
    {
        return ceil(
            $args['query']->found_posts /
                $this->DB_Settings_General->get_num_posts()
        );
    }
	...
	*/
    public function get_pagination_defaults()
    {
        return [
            ...
            'first_page_text' => esc_html__(
                apply_filters(
                    'wps_products_pagination_first_page_text',
                    'First'
                ),
                'wpshopify'
            ),
            'last_page_text' => esc_html__(
                apply_filters('wps_products_pagination_last_page_text', 'Last'),
                'wpshopify'
            ),
            'next_link_text' => esc_html__(
                apply_filters('wps_products_pagination_next_link_text', 'Next'),
                'wpshopify'
            ),
            'previous_link_text' => esc_html__(
                apply_filters('wps_products_pagination_prev_link_text', 'Prev'),
                'wpshopify'
            ),
            'show_posts_links' => apply_filters(
                'wps_products_pagination_show_as_prev_next',
                false
            ),
		...
		*/
        if ($args['show_posts_links'] === false) {
            ...
            if ($max_pages === 1) {
				...
				*/
                ...
                if ($check_range === true) {
                    ...
                    if (!in_array($current_page, $exclude)) {
                        $range_numbers = range(
                            $current_page - $mid_range,
                            $current_page + $mid_range
                        );
                    } elseif (
                        in_array($current_page, $start_range) &&
                        $current_page - $mid_range <= 0
                    ) {
                        $range_numbers = range(1, $args['range']);
                    } elseif (
                        in_array($current_page, $end_range) &&
                        $current_page + $mid_range >= $max_pages
                    ) {
                        $range_numbers = range(
                            $max_pages - $args['range'] + 1,
                            $max_pages
                        );
                    }
                }
				...
				*/
                foreach ($range_numbers as $page_number) {
                    if ($page_number == $current_page) {
                        ...
                        $this->Templates->Template_Loader
                            ->set_template_data([
                                'page_number' => $page_number,
                            ])
                            ->get_template_part(
                                'components/pagination/page-number',
                                'current'
                            );
                        ...
                        $page_href = $this->get_page_number_link(
                            $args,
                            $page_number
                        );
				...
				*/
                if ($current_page !== 1) {
                    $page_href_previous = $this->get_page_number_link(
                        $args,
                        $current_page - 1
                    );
				...
				*/
                if ($current_page !== $max_pages) {
                    $page_href_next = $this->get_page_number_link(
                        $args,
                        $current_page + 1
                    );
				...
				*/
                if (!in_array($max_pages, $range_numbers)) {
                    $page_href_last = $this->get_page_number_link(
                        $args,
                        $max_pages
                    );
			...
			*/
            $paginated_text = apply_filters(
                'wps_products_pagination_start',
                '<div itemscope itemtype="https://schema.org/SiteNavigationElement" class="wps-products-pagination">'
            );
            $paginated_text .= previous_posts_link(
                '<div class="wps-pagination-products-prev-link">' .
                    $args['previous_link_text'] .
                    '</div>'
            );
            $paginated_text .= next_posts_link(
                '<div class="wps-pagination-products-next-link">' .
                    $args['next_link_text'] .
                    '</div>',
                $max_pages
            );
